...
void checkpt_particle_bc_internal(const particle_bc_t *RESTRICT pbc)
{
    CHECKPT(pbc, 1);
...
void delete_particle_bc_list(particle_bc_t *pbc_list)
{
    particle_bc_t *pbc;
    while (pbc_list)
    {
                   ...
                   particle_bc_t **pbc_list)
{
    if (!pbc || !pbc_list)
        ERROR(("Bad args"));
    if (pbc->next)
        ERROR(("Particle boundary condition already in a list"));
...
get_particle_bc_id(particle_bc_t *pbc)
{
    if (!pbc)
        return 0;
    if (pbc == (particle_bc_t *)absorb_particles)
        return absorb_particles;
    if (pbc == (particle_bc_t *)reflect_particles)
        return reflect_particles;
    return pbc->id;
}
