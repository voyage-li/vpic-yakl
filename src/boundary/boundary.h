...
void checkpt_pbd(pb_diagnostic_t *diag);

pb_diagnostic_t *
restore_pbd(void);

void delete_pbd(pb_diagnostic_t *diag);
...
void pbd_buff_to_disk(pb_diagnostic_t *diag);
                         ...
                         const int i)
{
    ...
    if (diag->write_ux)
        buff[store++] = kpart(i, particle_var::ux);
    if (diag->write_uy)
        buff[store++] = kpart(i, particle_var::uy);
    if (diag->write_uz)
        buff[store++] = kpart(i, particle_var::uz);

    if (diag->write_momentum_magnitude)
    {
        buff[store++] = sqrt(pow(kpart(i, particle_var::ux), 2) + pow(kpart(i, particle_var::uy), 2) + pow(kpart(i, particle_var::uz), 2));
    }
...
#define global_pos_z ((k0 + (dz0 - 1) * 0.5) * grid->dz + grid->z0)
        if (diag->write_posx)
            buff[store++] = global_pos_x;
        if (diag->write_posy)
            buff[store++] = global_pos_y;
        if (diag->write_posz)
            buff[store++] = global_pos_z;
    }

    if (diag->write_weight)
        buff[store++] = kpart(i, particle_var::w);
...
void finalize_pb_diagnostic(species_t *sp);
...
void boundary_p_kokkos(particle_bc_t *RESTRICT pbc_list,
                       species_t *RESTRICT sp_list,
                       field_array_t *RESTRICT fa);
