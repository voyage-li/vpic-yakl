    ...
    field_array_t *RESTRICT fa)
{
        ...
        LIST_FOR_EACH(sp, sp_list)
        {
            ...
            for (; nm; pm--, nm--)
            {
                ...
                */
                ...
                // if( ((nn>=0) & (nn< rangel)) | ((nn>rangeh) & (nn<=rangem)) )
                std::cout << "nn " << nn << " rangel " << rangel << " rangeh " << rangeh << " rangem " << rangem << " voxel " << face << " old_nn " << old_nn << std::endl;
    ...
    do
    {
        ...
        do
        {
            ...
            {
                mp_end_recv(mp, f2b[face]);
                pi = (particle_injector_t *)(((char *)mp_recv_buffer(mp, f2b[face])) + 16);
                n = n_recv[face];
            }
            else
                continue;
    ...
    if (absorbed)
    {
        ...
        Kokkos::parallel_for(
            "Add rhob accumulation to device rhob", Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, n_fields), KOKKOS_LAMBDA(int i) {
    ...
    field_array_t *RESTRICT fa)
{
    ...
    do
    {
        ...
        LIST_FOR_EACH(sp, sp_list)
        {
            const auto &particle_send = sp->y_pc_i_h;
            ...
            for (; nm; pm--, nm--)
            {
                ...
                if (nn == absorb_particles)
                {
                    ...
                    // Ideally, we would batch all rhob accumulations together
                    // for efficiency
                    const auto &yrhob_accum_h = fa->y_f_rhob_accum_h;
                    const auto &yparticle_move_h = sp->y_pc_h;
                    const auto &yparticle_move_i_h = sp->y_pc_i_h;

                    float qsp = sp->q;

                    // Send the particle to the particle boundary diagnostic
                    if (sp->pb_diag->enable)
                        pbd_write_to_buffer(sp, yparticle_move_h, yparticle_move_i_h, copy_index);
                ...
                if (((nn >= 0) & (nn < rangel)) | ((nn > rangeh) & (nn <= rangem)))
                {
                    ...
                    // pi->dx=p0[i].dx;
                    // pi->dz=p0[i].dz;
                    pi->dx = sp->y_pc_h(copy_index, particle_var::dx);
                    pi->dy = sp->y_pc_h(copy_index, particle_var::dy);
                    pi->dz = sp->y_pc_h(copy_index, particle_var::dz);
                    ...
                    // pi->uy=p0[i].uy;
                    // pi->uz=p0[i].uz;
                    pi->ux = sp->y_pc_h(copy_index, particle_var::ux);
                    pi->uy = sp->y_pc_h(copy_index, particle_var::uy);
                    pi->uz = sp->y_pc_h(copy_index, particle_var::uz);

                    // pi->w=p0[i].w;
                    pi->w = sp->y_pc_h(copy_index, particle_var::w);
    ...
    do
    {
        ...
        do
        {
            ...
            for (; n; pi--, n--)
            {
                ...
                auto &particle_recv = sp_[id]->y_pr_h;
                auto &particle_recv_i = sp_[id]->y_pr_i_h;
                auto &particle_send = sp_[id]->y_pc_h;
                auto &particle_send_i = sp_[id]->y_pc_i_h;
    ...
    if (absorbed)
    {
        int n_fields = fa->g->nv;
        auto &yfd = fa->y_f_d;
        auto &yfad = fa->y_f_rhob_accum_d;
        auto &yfah = fa->y_f_rhob_accum_h;
        yfah.deep_copy_to(yfad);
        // Kokkos::deep_copy(yfad, yfah);

        yakl::c::parallel_for(
            yakl::c::Bounds<1>(0, n_fields),
            YAKL_LAMBDA(int i) { yfd(i, field_var::rhob) += yfad(i); });

        // Zero host accum array
        // Kokkos::deep_copy(yfah, 0.0f);
        yakl::c::parallel_for(
            yakl::c::Bounds<1>(0, yfah.size()),
            YAKL_LAMBDA(int i) { yfah(i) = 0.0f; })
    }
}

void boundary_p(particle_bc_t *RESTRICT pbc_list,
                species_t *RESTRICT sp_list,
                field_array_t *RESTRICT fa,
                accumulator_array_t *RESTRICT aa)
{
    ...
    do
    {
        ...
        do
        {
            ...
            {
                mp_end_recv(mp, f2b[face]);
                pi = (const particle_injector_t *)(((char *)mp_recv_buffer(mp, f2b[face])) + 16);
                n = n_recv[face];
            }
            else
                continue;
